...
__global__ void GetMeansKernel(unsigned char *indata, float *means, unsigned int nchans, unsigned int tsamps, unsigned int perthread) {

    int xidx = blockIdx.x * blockDim.x + threadIdx.x;
    int startidx = 0;

    if (xidx < tsamps / perthread) {
...
__global__ void RemoveZeroDmKernel(unsigned char *indata, float *means, unsigned int nchans, unsigned int tsamps, unsigned int perthread) {
